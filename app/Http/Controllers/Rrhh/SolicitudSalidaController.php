            ...
            case '1':
                ...
                // === VALIDAR POR CAMPO ===
                    ...
                    if(count($consulta3) > 0)
                    {
                        ...
                        // === FERIADOS, TOLERANCIAS Y HORARIOS CONTINUOS ===
                        else
                        {
                            $fh_salida  = $data1['f_salida'] . ' ' . $data1['h_salida'];
                            $fh_retorno = $data1['f_salida'] . ' ' . $data1['h_retorno'];
                            ...
                            foreach($consulta7 as $row7)
                            {
                                switch($row7['tipo_fthc'])
                                {
                                    // === FERIADO ===
                                    case '1':
                                        $respuesta['respuesta'] .= "En FERIDADO no se puede generar PAPELETA DE SALIDA.";
                                        return json_encode($respuesta);
                                        break;
                                    // === TOLERANCIA ===
                                    case '2':
                                    ...
                                    // === HORARIO CONTINUO ===
                                    case '3':
                                        $consulta8 = RrhhHorario::where('id', '=', $row7['horario_id'])
                                            ->select('h_ingreso', 'h_salida', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado', 'domingo')
                                            ->first();

                                        if($data1['h_retorno'] != '')
                                        {
                                            $fh_horario_1_ingreso = $data1['f_salida'] . ' ' . $consulta8['h_ingreso'];
                                            $fh_horario_1_salida  = $data1['f_salida'] . ' ' . $consulta8['h_salida'];
                                            ...
                                            {
                                                $respuesta_horario .= "HORARIO CONTINUO: La hora de salida y retorno debe de estar compredido entre " . $consulta8['h_ingreso'] . " y " . $consulta8['h_salida'] . ".";
                                            }
                                        }
                                        break;
                                    default:
